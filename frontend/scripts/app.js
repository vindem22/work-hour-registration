...
function personalData(data) {
  const pageName = document.querySelector('.home-all__people');
  user = data.email;
...
const addRows = function(response) {
    ...
    `
    let editBtn = `
      <button class="home-second__btn btn1" onclick="updateRecord()">
        <svg
          width="16"
          height="16"
          viewBox="0 0 16 16"
    ...
    for(let i=0;i < total;i++) {
      ...
      if(i >= total - 7) {
        totalHoursWeek += work_hours;
      }
      if(i >= total - 30) {
        totalHoursMonth += work_hours;
      }
      if(i === total - 1) {
        todayWorkedHours = work_hours;
      }
      ...
      let newRecord = `
        <tr class="all">
          <td class="home-second__td home-second__td--first">
              ...
              <p class="home-second__email">${user}</p>
          </td>
    ...
    let flexStatus = 40 - totalHoursWeek;
    let newStatistics = `
      <li class="home-second__item">
        <p class="home-second__text">Today</p>
        <p class="home-second__suntext">${todayWorkedHours}h</p>
      </li>
      <li class="home-second__item">
        <p class="home-second__text">This week</p>
        <p class="home-second__suntext">${totalHoursWeek}h</p>
      </li>
      <li class="home-second__item">
        <p class="home-second__text">This month</p>
        <p class="home-second__suntext">${totalHoursMonth}h</p>
      </li>
      <li class="home-second__item">
        <p class="home-second__text">${flexStatus > 0 ? "Required" : "Relax"}</p>
        <p class="home-second__suntext">${Math.abs(flexStatus)}h</p>
      </li>
    `
    const statBar = document.getElementById('statistics-bar');
    console.log(statBar)
    statBar.insertAdjacentHTML('afterbegin', newStatistics);
}

const createRecord = function() {
  //const token = localStorage.getItem('auth-token');
  const token = 'Tzr1W38GLulSde5xG5U0VtaWgYvvLo'
  console.log(token)
  ...
  const requestBody = {
      employee_id : 1,
      arrive_time : inputs[0].value,
      exit_time : inputs[1].value,
      status : 1
  }
  ...
  if(inputs[2].value.length > 0) {
    ...
    fetch('http://localhost:8000/absense_record/record_create/', { method: "post", headers: {
    'Content-Type': 'application/json;charset=utf-8','Authorization': 'Bearer ' + token
  },body: JSON.stringify(reqBody)}).catch(e => console.log(e))
  }
