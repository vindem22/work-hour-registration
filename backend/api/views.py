...
from api.serializers import EmployeeSerializer, RecordCreateSerializer, RecordSerializer
...
class RecordViewSet(viewsets.GenericViewSet):  
    ...
    #     record = Employee.objects.get(id=id).records
    #     ser = RecordSerializer(record,many=True).data
    #     return Response({'data': ser.data}, status=status.HTTP_200_OK)
